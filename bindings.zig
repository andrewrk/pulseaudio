...
extern fn pa_proplist_key_valid(key: [*c]const u8) c_int;
extern fn pa_proplist_sets(p: ?*proplist, key: [*c]const u8, value: [*c]const u8) c_int;
extern fn pa_proplist_setp(p: ?*proplist, pair: [*c]const u8) c_int;
extern fn pa_proplist_setf(p: ?*proplist, key: [*c]const u8, format: [*c]const u8, ...) c_int;
extern fn pa_proplist_set(p: ?*proplist, key: [*c]const u8, data: ?*const anyopaque, nbytes: usize) c_int;
extern fn pa_proplist_gets(p: ?*const proplist, key: [*c]const u8) [*c]const u8;
extern fn pa_proplist_get(p: ?*const proplist, key: [*c]const u8, data: [*c]?*const anyopaque, nbytes: [*c]usize) c_int;
extern fn pa_proplist_update(p: ?*proplist, mode: update_mode_t, other: ?*const proplist) void;
extern fn pa_proplist_unset(p: ?*proplist, key: [*c]const u8) c_int;
extern fn pa_proplist_unset_many(p: ?*proplist, keys: [*c]const [*c]const u8) c_int;
extern fn pa_proplist_iterate(p: ?*const proplist, state: [*c]?*anyopaque) [*c]const u8;
extern fn pa_proplist_to_string(p: ?*const proplist) [*c]u8;
extern fn pa_proplist_to_string_sep(p: ?*const proplist, sep: [*c]const u8) [*c]u8;
extern fn pa_proplist_from_string(str: [*c]const u8) ?*proplist;
extern fn pa_proplist_contains(p: ?*const proplist, key: [*c]const u8) c_int;
